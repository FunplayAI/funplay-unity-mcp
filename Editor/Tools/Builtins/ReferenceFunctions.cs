...
namespace Funplay.Editor.Tools.Builtins
{
    ...
    internal static class ReferenceFunctions
    {
        [Description("Find asset references in both directions. 'depends_on' = the target's direct forward " +
                     ...
                     "scan of every 'Assets/' asset path asking which ones directly depend on the target. The reverse " +
                     "scan is O(N) over all project assets and can be slow on large projects (expected); results are " +
                     "capped at max_results (truncated=true when the cap is hit) and 'scanned' reports how many asset " +
                     "paths were examined. Not cached to disk.")]
        [ReadOnlyTool]
